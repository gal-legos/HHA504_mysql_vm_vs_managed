...
VM_DB_PORT = int(os.getenv("VM_DB_PORT", "3306"))
...
# Remove the ?ssl=false from URL and handle SSL in connect_args instead
server_url = f"mysql+pymysql://{VM_DB_USER}:{VM_DB_PASS}@{VM_DB_HOST}:{VM_DB_PORT}/{VM_DB_NAME}"
print("[STEP 1] Connecting to MySQL server:", server_url.replace(VM_DB_PASS, "*****"))
t0 = time.time()

# Create engine with SSL disabled via connect_args
...
print("[STEP 2] Connected to database:", VM_DB_NAME)

# --- 3) Create a DataFrame and write to a table ---
table_name = "visits"
print(f"[STEP 3] Creating table '{table_name}' and inserting data...")
...
# Write DataFrame to SQL - no need for connect_args here, engine already configured
...
with engine.connect() as conn:
    ...
    print("\n[STEP 4b] Sample data from table:")
    data_df = pd.read_sql(f"SELECT * FROM `{table_name}` LIMIT 5", con=conn)
    print(data_df)
