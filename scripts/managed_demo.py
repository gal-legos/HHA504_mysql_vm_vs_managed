...
MAN_DB_PORT = int(os.getenv("MAN_DB_PORT", "3306"))  # Convert to int
...
# Remove ?ssl=false from URL and use connect_args instead
server_url = f"mysql+pymysql://{MAN_DB_USER}:{MAN_DB_PASS}@{MAN_DB_HOST}:{MAN_DB_PORT}/{MAN_DB_NAME}"
print("[STEP 1] Connecting to Managed MySQL:", server_url.replace(MAN_DB_PASS, "*****"))
t0 = time.time()

# Use connect_args to properly disable SSL
...
print("[STEP 2] Connected to database:", MAN_DB_NAME)
...
print(f"[OK] Wrote {len(df)} rows to table '{table_name}'")
